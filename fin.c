...
FINopen( const char * filename, int main_flag )
{
    ...
    if ( ( filename[0] == '-' && filename[1] == 0 ) || strcmp( filename, "/dev/stdin" ) == 0 ) {
#if MSDOS
        if ( bm )
            setmode( 0, O_BINARY );
#endif
...
enlarge_fin_buffer( FIN * fin )
{
    ...
    fin->start = fin->buff = (char *)erealloc( fin->buff, newsize );
    fin->buffsz            = newsize;
...
/* called by open_main() if open_flag is on  */
{
    ...
    /* real failure */
    {
        ...
        static FIN  dead_main = { 0, (FILE *)0, &dead_buff, &dead_buff, &dead_buff, 0, EOF_FLAG };
